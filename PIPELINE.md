...
Calls Claude (Opus) once per *file* to write the long block that goes below the code.
...
1. **Read the failing step's log.** The model steps fail fast: the first failure stops the
...
Classification ran on Sonnet until 2026-09, and on Haiku since, to cut usage (lint too). It emits six fields, two of them constrained to a
