...
public class Solution
{
    public int LastStoneWeight(int[] stones)
    {
        ...
        {
            stoneQueue.Enqueue(stones[i], -stones[i]);
        }

        while (stoneQueue.Count > 1)
        {
            int first = stoneQueue.Dequeue();
            int second = stoneQueue.Dequeue();
            ...
            else
            {
                ...
                stoneQueue.Enqueue(newStone, -newStone);
            }
        }
        bool hasElement = stoneQueue.TryPeek(out int element, out _);
...
WHY TRYPEEK AND NOT PEEK
  ...
  stoneQueue.Peek(). On an input like [1,1] or [2,7,4,1,8,1] the cancellation
