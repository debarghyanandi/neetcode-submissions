...
| step | model | why |
|---|---|---|
| `lint` | `dotnet format whitespace` + Haiku | Two halves. **Spacing is deterministic** - `dotnet format whitespace --folder`, config in `.editorconfig`, preinstalled on the runner, zero tokens, runs over every file including ones no model call touches. **Names are the model**, and `sameShape()` catches any dishonesty - from the formatter too, which is checked exactly like the model. Thinking capped at 600 via `MAX_THINKING_TOKENS` (was 2000, which was a ceiling rather than a limit); no `--effort` flag on Haiku. Override with `LINT_THINKING_TOKENS`. |
